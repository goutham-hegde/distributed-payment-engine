 ...
 *
 * <p>payment-gateway cannot mint a token any more. {@code TokenIssuer} moved into
 * payment-orchestrator at M5 part 2 along with the private key, and there is no import that brings
 * it back - which is precisely the property the milestone was for.
 *
 * <p>This service has no security tests of its own; it needs the key only so that its context can
 * build a {@code JwtDecoder} at all. A copy of account-service's class rather than a shared test
 * jar: one small file duplicated across two modules is cheaper than publishing test artifacts, and
 * the duplication is visible where a {@code test-jar} dependency would not be.
 *
